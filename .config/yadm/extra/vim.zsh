...
function vim_setup() {
  ...
  zinit ice lucid wait cloneonly proto"git" nocompile notify \
    atclone"install --compare -D colors/desertink.vim $VIM_COLOR/desertink.vim" \
    atpull"%atclone"
  zinit light toupeira/vim-desertink

  zinit ice lucid wait cloneonly proto"git" nocompile \
  ...
  zinit ice lucid wait cloneonly proto"git" nocompile \
    atclone"rsync --mkpath --exclude '.*' --archive . $VIM_PACK/editorconfig" \
    atpull"%atclone"
